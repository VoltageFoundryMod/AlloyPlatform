...
way to say "this shape again" that nanosvg understands. The tiled texture above
expands to ~12 MB, which is then parsed on every module instantiation and
shipped inside the .vcvplugin. The tool reports the growth so it cannot happen
quietly; if the number is alarming, the fix is fewer or simpler tiles in the
drawing, not a cleverer exporter.
...
def _editor_cruft(el):
    ...
    copy — and paying that 1927 times makes the file Inkscape has to *parse*
