...
static uint8_t           sHistoryCount
    = 0; // entries stored so far (saturates at HISTORY_SIZE)
static uint8_t sHistoryHead = 0; // index where the NEXT entry will be written
static int8_t  sHistoryPos
    = 0; // browsing position: 0 = current (unsaved) line, 1 = most recent
