...
⚠️ **Only the three `purpose: any` PWA icons are round (`ROUND_ICONS` in `gen-icons.mjs`); every other icon is square deliberately.** `maskable-icon-512x512.png` is under a contract with the launcher — we supply a full-bleed square and *it* crops to circle, squircle, rounded square or teardrop, so pre-rounding exposes the transparent corners as bright wedges on any non-circular mask and double-crops the mark. The apple-touch icon is square because iOS masks it itself and flattens alpha onto black. The iOS app icon is square **and must have no alpha channel at all** — App Store Connect rejects such an icon at upload, not at review. Everything in `web-configurator/public/` is generated output: hand-editing an icon there survives exactly until the next `make web-icons`.
