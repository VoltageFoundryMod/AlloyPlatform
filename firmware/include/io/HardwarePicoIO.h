...
class HardwarePicoIO : public IHardwareIO {
  public:
    ...
    float readPot(PotId id) override {
        ...
        case PotId::RELATION:
            return gRelation / 24.0f; // 0–24 st → 0–1
        case PotId::SHAPE:
            return gShape; // already 0–1
        case PotId::MOTION:
            return gMotion; // already 0–1
        case PotId::COLOR:
            return gColor; // already 0–1
        case PotId::CURVE:
    ...
    bool isPatched(CVId id) override {
        switch (id) {
        case CVId::VOCT:
            return false; // no V/Oct jack yet
        case CVId::GATE:
            return gGatePatched; // set by MIDI / button combo
        default:
            return false;
        }
    }
    ...
    bool readButton(ButtonId id) override {
        switch (id) {
        case ButtonId::MODE:
            return _btnMode.isDown();
        case ButtonId::SHIFT:
            return _btnShift.isDown();
        default:
            return false;
        }
    }
